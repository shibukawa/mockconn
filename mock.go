...
var (
	errorColorLabel   = color.RedString("Error")
	ngColorLabel      = color.RedString("NG")
	okColorLabel      = color.GreenString("OK")
	errorNoColorLabel = "Error"
	ngNoColorLabel    = "NG"
	okNoColorLabel    = "OK"
...
func cyan(value []byte, useColor bool) string {
	if useColor {
...
func yellow(value []byte, useColor bool) string {
	if useColor {
...
func stack(t *testing.T) string {
	...
	return fmt.Sprintf("\nStack:\n%s", magentaColor(string(debug.Stack())))
}
...
func (c *Conn) Verify() []error {
	...
	case ReadActionType:
		...
		if len(read.data) > 0 {
			c.addError(fmt.Errorf("%s: mock socket scenario %d - there is remained data to read: %s", c.errorLabel(), c.current+1, yellow(read.data, useColor)))
		} else {
			c.current++
		}
	case WriteActionType:
		write := current.(*writeAction)
		c.addError(fmt.Errorf("%s: mock socket scenario %d - there is remained data to write: %s", c.errorLabel(), c.current+1, yellow(write.data, useColor)))
	}
	if c.current < len(c.scenario) {
		c.addError(fmt.Errorf("%s: Unconsumed senario exists - %d/%d", c.errorLabel(), len(c.scenario)-c.current, len(c.scenario)))
	}
	result := c.errors
	c.errors = errors
		...
		for i := 0; i < len(c.scenario); i++ {
			...
			if i >= c.current {
				if c.t != nil {
					...
					result = ngNoColorLabel
				}
			} else {
				if c.t != nil {
					result = okColorLabel
				} else {
					result = okNoColorLabel
				}
				ok = true
			}
			switch current.Type() {
			case ReadActionType:
				read := current.(*readAction)
				fmt.Fprintf(&buffer, "%s (%d) Read(): %s\n", result, i+1, logText(ok, read.original, read.data, c.t))
			case WriteActionType:
				write := current.(*writeAction)
				fmt.Fprintf(&buffer, "%s (%d) Write(): %s\n", result, i+1, logText(ok, write.original, write.data, c.t))
...
func logText(ok bool, original, actual []byte, t *testing.T) string {
	useColor := (t != nil)
	if ok {
		if len(actual) != 0 && len(actual) != len(original) {
			return cyan(original[:len(original)-len(actual)], useColor) + yellow(actual, useColor)
		}
		return cyan(original, useColor)
	}
	return yellow(original, useColor)
}
...
func (c *Conn) Read(b []byte) (n int, err error) {
	...
	case WriteActionType:
		...
		if len(write.data) == 0 {
			...
			return 0, c.addError(fmt.Errorf("%s: socket scenario %d - should write, but Read() is called.%s", c.errorLabel(), c.current+1, stack(c.t)))
		}
	case CloseActionType:
		return 0, c.addError(fmt.Errorf("%s: socket scenario %d - should close, but Read() is called.%s", c.errorLabel(), c.current+1, stack(c.t)))
	}
	return 0, nil
}
...
func (c *Conn) Write(b []byte) (n int, err error) {
	...
	case ReadActionType:
		read := current.(*readAction)
		if len(read.data) > 0 {
			return 0, c.addError(fmt.Errorf("%s: socket scenario %d - should read data, but Write() is called.%s", c.errorLabel(), c.current+1, stack(c.t)))
		}
		c.current++
		return c.Write(b)
	case WriteActionType:
		...
		if len(b) <= len(write.data) {
			...
			if !same {
				return 0, c.addError(fmt.Errorf("%s: socket scenario %d - Write() expected=%s actual=%s.%s", c.errorLabel(), c.current+1, cyan(write.data, useColor), yellow(b, useColor), stack(c.t)))
			}
			if len(b) == len(write.data) {
				c.current++
			} else {
		...
		return 0, c.addError(fmt.Errorf("%s: socket scenario %d - Write() expected=%s actual=%s.%s", c.errorLabel(), c.current+1, cyan(write.data, useColor), yellow(b, useColor), stack(c.t)))
	case CloseActionType:
		return 0, c.addError(fmt.Errorf("%s: socket scenario %d - should close, but Write() is called.%s", c.errorLabel(), c.current+1, stack(c.t)))
	}
	return 0, nil
}
...
func (c *Conn) Close() error {
	...
	case ReadActionType:
		read := current.(*readAction)
		if len(read.data) > 0 {
			return c.addError(fmt.Errorf("%s: socket scenario %d - should read data, but Close() is called.%s", c.errorLabel(), c.current+1, stack(c.t)))
		}
		c.current++
		return c.Close()
	case WriteActionType:
		return c.addError(fmt.Errorf("%s: socket scenario %d - should write data, but Close() is called.%s", c.errorLabel(), c.current+1, stack(c.t)))
	case CloseActionType:
		c.current++
	}
	c.closed = true
...
func (c Conn) errorLabel() string {
	if c.t != nil {
		return errorColorLabel
	}
	return errorNoColorLabel
}
